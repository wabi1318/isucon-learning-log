...
ロードマップに複数人で行う操作がある場合、個人学習へ暗黙に置き換えません。

- 「1人が`curl`を実行し、別の人がアクセスログを追う」は、二つのターミナルを使い、ログ追跡とリクエスト送信を順番に自分で行う。元項目と個人用の実施方法を日次ログへ書き、`読み替え完了`とする
- 「環境構築状況と不明用語を共有する」は、個人用の状況と質問をまとめても、実際にチームへ共有した証拠がなければ共有完了にしない
- 「翌週までに使う過去問環境を確定する」は、候補を調べただけで完了にせず、利用者またはチームの決定を記録する

チーム作業を個人用に読み替えられない場合は`未確認`または`未実行`のまま残し、誰の判断または外部状態が必要かを`study-status.md`へ書きます。
